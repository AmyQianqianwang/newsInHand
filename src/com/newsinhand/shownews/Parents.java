...
public class Parents extends Activity {
	private static final String TAG = "Parents";
	private final static String ALBUM_PATH = Environment
			.getExternalStorageDirectory() + "/news_image/" + "parents/";
	Bitmap bitmap;
	...
	private Handler handler = new Handler(new Callback() {
		...
		public boolean handleMessage(Message msg) {
			...
			Toast.makeText(Parents.this, obj, Toast.LENGTH_SHORT)
					.show();
			return false;
		}
	});
	private Handler mHandler_initiate = new Handler() {
		public void handleMessage(Message msg) {
			...
			SimpleAdapter listItemAdapter = new SimpleAdapter(
					Parents.this, listItem,// 数据源
					R.layout.parents_item,// ListItem的XML实现
					// 动态数组与ImageItem对应的子项
					new String[] { "ItemImage", "ItemTitle", "ItemText" },
					// ImageItem的XML文件里面的一个ImageView,两个TextView ID
					new int[] { R.id.parents_image, R.id.parents_title,
							R.id.parents_digest });
	...
	protected void onCreate(Bundle pSavedInstanceState) {
		...
		mListView = (Refresh) findViewById(R.id.parents_list);
		listItem = new ArrayList<HashMap<String, Object>>();
		List<News> db_data = new ArrayList<News>();
		DB news_db = new DB(Parents.this);
				...
				new Thread(new Runnable() {
					...
					public void run() {
						try {
							...
							DB db_insert = new DB(Parents.this);
							db_insert.insert(mList);
							mList.clear();
							DB news_check = new DB(Parents.this);
				...
				new Thread(new Runnable() {
					...
					public void run() {
						...
						if (Main.isNetworkAvailable) {
							...
							Data.putString("news_sort", "亲子");
							Intent intent = new Intent(Parents.this,
									Webview.class);
						...
						public void run() {
							try {
								...
								for (int i = 0; i < mList.size(); i++) {
									...
									final String title = ns.getTitle();
									// 将网络图片存放在本地
									if (!tool.isExist("news_image/parents/"
											+ title + ".jpeg")) {
										try {
											URL url = new URL(image);
											InputStream is = url.openStream();
