...
public class Body extends Activity {
	...
	private final static String ALBUM_PATH = Environment
			.getExternalStorageDirectory() + "/news_image/" + "body/";
	Bitmap bitmap;
	...
	private Handler handler = new Handler(new Callback() {
		...
		public boolean handleMessage(Message msg) {
			...
			Toast.makeText(Body.this, obj, Toast.LENGTH_SHORT)
					.show();
			return false;
		}
	});
	private Handler mHandler_initiate = new Handler() {
		public void handleMessage(Message msg) {
			...
			SimpleAdapter listItemAdapter = new SimpleAdapter(
					Body.this, listItem,// 数据源
					R.layout.body_item,// ListItem的XML实现
					// 动态数组与ImageItem对应的子项
					new String[] { "ItemImage", "ItemTitle", "ItemText" },
					// ImageItem的XML文件里面的一个ImageView,两个TextView ID
					new int[] { R.id.body_image, R.id.body_title,
							R.id.body_digest });
	...
	private Handler mHandler_refresh = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			for (int i = 0; i < 10; i++) {
				...
				map.put("ItemTitle", ns.getTitle());
				System.out.println("身体" + ns.getId());
				map.put("ItemText", ns.getDesc());
				map.put("ItemLink", ns.getLink());
				listItem.add(map);
			}
	...
	protected void onCreate(Bundle pSavedInstanceState) {
		...
		mListView = (Refresh) findViewById(R.id.body_list);
		listItem = new ArrayList<HashMap<String, Object>>();
		List<News> db_data = new ArrayList<News>();
		DB news_db = new DB(Body.this);
				...
				new Thread(new Runnable() {
					...
					public void run() {
						try {
							...
							DB db_insert = new DB(Body.this);
							db_insert.insert(mList);
							mList.clear();
							DB news_check = new DB(Body.this);
				...
				new Thread(new Runnable() {
					...
					public void run() {
						...
						if (Main.isNetworkAvailable) {
							...
							Data.putString("news_sort", "身体");
							Intent intent = new Intent(Body.this,
									Webview.class);
						...
						public void run() {
							try {
								...
								for (int i = 0; i < mList.size(); i++) {
									...
									final String title = ns.getTitle();
									// 将网络图片存放在本地
									if (!tool.isExist("news_image/body/"
											+ title + ".jpeg")) {
										try {
											URL url = new URL(image);
											InputStream is = url.openStream();
