...
public class China extends Activity {
	private static final String TAG = "China";
	private final static String ALBUM_PATH = Environment
			.getExternalStorageDirectory() + "/news_image/" + "china/";
	Bitmap bitmap;
	...
	private Handler handler = new Handler(new Callback() {
		...
		public boolean handleMessage(Message msg) {
			...
			Toast.makeText(China.this, obj, Toast.LENGTH_SHORT)
					.show();
			return false;
		}
	});
	private Handler mHandler_initiate = new Handler() {
		public void handleMessage(Message msg) {
			...
			SimpleAdapter listItemAdapter = new SimpleAdapter(
					China.this, listItem,// 数据源
					R.layout.china_item,// ListItem的XML实现
					// 动态数组与ImageItem对应的子项
					new String[] { "ItemImage", "ItemTitle", "ItemText" },
					// ImageItem的XML文件里面的一个ImageView,两个TextView ID
					new int[] { R.id.china_image, R.id.china_title,
							R.id.china_digest });
	...
	private Handler mHandler_refresh = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			for (int i = 0; i < 10; i++) {
				...
				map.put("ItemTitle", ns.getTitle());
				System.out.println("国内" + ns.getId());
				map.put("ItemText", ns.getDesc());
				map.put("ItemLink", ns.getLink());
				listItem.add(map);
			}
	...
	protected void onCreate(Bundle pSavedInstanceState) {
		...
		mListView = (Refresh) findViewById(R.id.china_list);
		listItem = new ArrayList<HashMap<String, Object>>();
		List<News> db_data = new ArrayList<News>();
		DB news_db = new DB(China.this);
				...
				new Thread(new Runnable() {
					...
					public void run() {
						try {
							...
							DB db_insert = new DB(China.this);
							db_insert.insert(mList);
							mList.clear();
							DB news_check = new DB(China.this);
				...
				new Thread(new Runnable() {
					...
					public void run() {
						...
						if (Main.isNetworkAvailable) {
							...
							Data.putString("news_sort", "国内");
							Intent intent = new Intent(China.this,
									Webview.class);
							...
							try {
								...
								for (int i = 0; i < div_1.size(); i++) {
									...
									title = div_1.get(i).select(".linkto")
											.text()
											+ " (国内)";
									//System.out.println(title+"222222222222222222222222222222***********************************************************");
									image = div_1.get(i).select(".picto")
											.attr("src");
									link = "http://news.qq.com"
											+ div_1.get(i).select(".linkto")
								...
								for (int i = 0; i < mList.size(); i++) {
									...
									final String title = ns.getTitle();
									// 将网络图片存放在本地
									if (!tool.isExist("news_image/china/"
											+ title + ".jpeg")) {
										try {
											URL url = new URL(image);
											InputStream is = url.openStream();
