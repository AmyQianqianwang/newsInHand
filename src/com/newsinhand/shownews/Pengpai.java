...
public class Pengpai extends Activity {
	private static final String TAG = "Pengpai";
	private final static String ALBUM_PATH = Environment
			.getExternalStorageDirectory() + "/news_image/" + "pengpai/";
	Bitmap bitmap;
	...
	private Handler handler = new Handler(new Callback() {
		...
		public boolean handleMessage(Message msg) {
			...
			Toast.makeText(Pengpai.this, obj, Toast.LENGTH_SHORT)
					.show();
			return false;
		}
	});
	private Handler mHandler_initiate = new Handler() {
		public void handleMessage(Message msg) {
			...
			SimpleAdapter listItemAdapter = new SimpleAdapter(
					Pengpai.this, listItem,// 数据源
					R.layout.pengpai_item,// ListItem的XML实现
					// 动态数组与ImageItem对应的子项
					new String[] { "ItemImage", "ItemTitle", "ItemText" },
					// ImageItem的XML文件里面的一个ImageView,两个TextView ID
					new int[] { R.id.pengpai_image, R.id.pengpai_title,
							R.id.pengpai_digest });
	...
	private Handler mHandler_refresh = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			for (int i = 0; i < 10; i++) {
				...
				map.put("ItemTitle", ns.getTitle());
				System.out.println("澎湃新闻" + ns.getId());
				map.put("ItemText", ns.getDesc());
				map.put("ItemLink", ns.getLink());
				listItem.add(map);
			}
	...
	protected void onCreate(Bundle pSavedInstanceState) {
		...
		setContentView(R.layout.pengpai);

		mListView = (Refresh) findViewById(R.id.pengpai_list);
		listItem = new ArrayList<HashMap<String, Object>>();
		List<News> db_data = new ArrayList<News>();
		DB news_db = new DB(Pengpai.this);
				...
				new Thread(new Runnable() {
					...
					public void run() {
						try {
							...
							//System.out.println(div_1.text()+"div_1.text**************************************");
						//System.out.println(div_2.text()+"div_2.text**************************************");
						//System.out.println(div_1.text()+"#####################################################");
							//System.out.println("pengpai news:");
							...
							DB db_insert = new DB(Pengpai.this);
							db_insert.insert(mList);
							mList.clear();
							DB news_check = new DB(Pengpai.this);
				...
				new Thread(new Runnable() {
					...
					public void run() {
						...
						if (Main.isNetworkAvailable) {
							...
							Data.putString("news_sort", "澎湃");
							Intent intent = new Intent(Pengpai.this,
									Webview.class);
						...
						public void run() {
							...
							System.out.println(div_1.text()+"#####################################################");*/
								...
								for (int i = 0; i < mList.size(); i++) {
									...
									final String title = ns.getTitle();
									// 将网络图片存放在本地
									if (!tool.isExist("news_image/pengpai/"
											+ title + ".jpeg")) {
										try {
											URL url = new URL(image);
											InputStream is = url.openStream();
