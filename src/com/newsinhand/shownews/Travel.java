...
public class Travel extends Activity {
	...
	private final static String ALBUM_PATH = Environment
			.getExternalStorageDirectory() + "/news_image/" + "travel/";
	...
	private Handler handler = new Handler(new Callback() {
		...
		public boolean handleMessage(Message msg) {
			...
			Toast.makeText(Travel.this, obj, Toast.LENGTH_SHORT).show();
			return false;
		}
	});
	private Handler mHandler_initiate = new Handler() {
		public void handleMessage(Message msg) {
			...
			for (int i = 0; i < 10; i++) {
				...
				System.out.println("旅游" + ns.getId());
			...
			SimpleAdapter listItemAdapter = new SimpleAdapter(
					Travel.this, listItem,// 数据源
					R.layout.travel_item,// ListItem的XML实现
					// 动态数组与ImageItem对应的子项
					new String[] { "ItemImage", "ItemTitle", "ItemText" },
					// ImageItem的XML文件里面的一个ImageView,两个TextView ID
					new int[] { R.id.travel_image , R.id.travel_title,
							R.id.travel_digest });
	...
	protected void onCreate(Bundle pSavedInstanceState) {
		...
		mListView = (Refresh) findViewById(R.id.travel_list);
		listItem = new ArrayList<HashMap<String, Object>>();
		List<News> db_data = new ArrayList<News>();
		DB news_db = new DB(Travel.this);
				...
				new Thread(new Runnable() {
					...
					public void run() {
						...
						//System.out.println(div_1.text()+"#####################################################");
							System.out.println("travel news:");
							...
							DB db_insert = new DB(Travel.this);
							db_insert.insert(mList);
							mList.clear();
							DB news_check = new DB(Travel.this);
					...
					while (state) {
						...
						if (Main.travel_state == true) {
							for (int i = 0; i < mList.size(); i++) {
								...
								// 将网络图片存放在本地
								if (!tool.isExist("news_image/travel/"
										+ title + ".jpeg")) {
									try {
				...
				new Thread(new Runnable() {
					...
					public void run() {
						...
						if (Main.isNetworkAvailable) {
							...
							Data.putString("news_sort", "旅游");
							Intent intent = new Intent(Travel.this,
									Webview.class);
